...
<main class="flex-1 overflow-y-auto p-6">
    <h1 class="text-center font-bold text-2xl mb-6">Quản Lý Digital Resource Major</h1>
    ...
    <div class="overflow-x-auto">
        ...
        <table class="table-auto w-full border-collapse border border-gray-300">
            ...
            <tbody>
                @forelse ($digitalresources as $digitalresource)
                    <tr class="hover:bg-gray-100">
                        <td class="border border-gray-300 px-4 py-2 text-center">
                            {{ $digitalresource->tailieumo->ten_tai_lieu}}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-center">
                            {{ $digitalresource->nganh->ten_nganh}}
                        </td>
                    </tr>
                @empty
                    <tr>
    ...
    <div x-data="{ open: @entangle('isModalOpen') }" x-show="open"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <!-- Form -->
            <form wire:submit.prevent="{{ $isEditMode ? 'updateDigitalResource' : 'createDigitalResource' }}"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <div class="flex justify-center space-x-4">
                <button type="button" wire:click="closeConfirmModal"
                    class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Huỷ</button>
                <button type="button" wire:click="deleteDigitalResource"
    ...
    <div class="flex justify-center mt-6">
        <div class="inline-flex items-center space-x-2">
            ...
            @if($digitalresources->onFirstPage())
                <span class="px-4 py-2 text-gray-400 bg-gray-200 rounded-md cursor-not-allowed">Previous</span>
            @else
                <a href="{{ $digitalresources->previousPageUrl() }}"
                    class="px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700">Previous</a>
            @endif

            <!-- Page Numbers -->
            @foreach ($digitalresources->getUrlRange(1, $digitalresources->lastPage()) as $page => $url)

                <a wire:click.prevent="gotoPage({{ $page }})" href="#"
                    class="{{ $page == $digitalresources->currentPage() ? 'bg-blue-600 text-white' : 'text-blue-600 border border-gray-300 hover:bg-gray-100' }} px-4 py-2 rounded-md">
                    {{ $page }}
                </a>
            @endforeach

            <!-- Next Page Button -->
            @if($digitalresources->hasMorePages())
                <a href="{{ $digitalresources->nextPageUrl() }}"
